...
/*   Updated: 2023/01/11 22:35:31 by satushi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	bool	create_samephilo(t_allinfo *info)
{
	...
	{
		((info->philoinfo)[l_f_n - 1]).fork_info.r_fork = l_f_n - 1;
		((info->philoinfo)[l_f_n - 1]).fork_info.l_fork = l_f_n;
		((info->philoinfo)[l_f_n - 1]).how_eated = 0;
		((info->philoinfo)[l_f_n - 1]).number_of_philo = l_f_n;
		((info->philoinfo)[l_f_n - 1]).time_to_die = 0;
		l_f_n++;
	}
	((info->philoinfo)[0]).fork_info.r_fork = info->philo_num;
	((info->philoinfo)[0]).fork_info.l_fork = 1;
